...
function ommu_setup() {
	...
	add_editor_style( array( 'style.css', ommu_fonts_url() ) );
	...
	add_theme_support('ommu-backgrounds');
	add_theme_support('ommu-fonts');
}
add_action( 'after_setup_theme', 'ommu_setup' );
...
add_action( 'wp_enqueue_scripts', 'ommu_scripts_styles' );
 ...
 */
function ommu_fonts() {
	$fonts_url = ommu_fonts_url();
	if ( ! empty( $fonts_url ) )
		wp_enqueue_style( 'ommu-fonts', esc_url_raw( $fonts_url ), array(), null );
}
add_action( 'wp_enqueue_scripts', 'ommu_fonts' );
...
add_filter( 'wp_title', 'ommu_wp_title', 10, 2 );
...
function ommu_active_nav_class( $classes, $item ) {
    if ( $item->current == 1 || $item->current_item_ancestor == true ) {
        $classes[] = 'active';
    }
    return $classes;
}
add_filter( 'nav_menu_css_class', 'ommu_active_nav_class', 10, 2 );
...
add_filter( 'wp_list_pages', 'ommu_active_list_pages_class', 10, 2 );
...
add_action( 'widgets_init', 'ommu_widgets_init' );
...
function ommu_front_sidebar_class() {
	$count = 0;
...
function ommu_footer_sidebar_class() {
	$count = 0;
...
if ( ! function_exists( 'ommu_content_nav' ) ) :
/**
...
if ( ! function_exists( 'ommu_comment' ) ) :
/**
 ...
 * Create your own ommu_entry_date() to override in a child theme.
...
function ommu_body_class( $classes ) {
	...
	if ( wp_style_is( 'ommu-fonts', 'queue' ) )
...
add_filter( 'body_class', 'ommu_body_class' );
...
add_action( 'template_redirect', 'ommu_content_width' );
...
add_filter( 'post_class', 'ommu_filter_post_class', 20 );
...
add_action( 'widgets_init', 'ommu_remove_recent_comments_style' );
